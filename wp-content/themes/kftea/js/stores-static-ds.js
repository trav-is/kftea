...
function StoresDataSource() {
  ...
  var that = this;
  $.get('/wp-content/themes/kftea/js/stores.csv', function(data) {
 ...
 * @private
 */
StoresDataSource.prototype.FEATURES_ = new storeLocator.FeatureSet(
 ...
 */
StoresDataSource.prototype.parse_ = function(csv) {
  var stores = [];
  var rows = csv.split('\n');
  ...
  for (var i = 1, row; row = rows[i]; i++) {
    ...
    var locality = this.join_([row.Locality, row.State, row.Postcode], ', ');
