...
add_image_size('product-image3',150,175, true);
...
function product_shortcode($atts) {
	...
	if ($view != 'list' && $featured != 'yes'){
		$product_shortcode .= '<div class="products">';
	}

	if ($view != 'list' && $featured == 'yes'){
		$product_shortcode .= '<div class="products" style="background: #eee;">';
	}
	...
	while($grid_products->have_posts()): $grid_products->the_post();
		...
		if ($view == 'list') {
			...
			$theimage = wp_get_attachment_image_src(get_post_thumbnail_id($productID), 'product-image3');

			$product_shortcode .= '<div class="product">';
			$product_shortcode .= '<h2>' . get_the_title() . '</h2>';
			$product_shortcode .= '<p class="product-excerpt">' . get_the_excerpt() . '</p>';
			...
			$product_shortcode .= '<div class="product-image"><img src="' . $theimage[0] . '" alt="' . get_the_title() . '" /></div>';
			...
			$product_shortcode .= '<a href="#" class="desc-btn">Flavors &amp; Description</a>';
			
			$product_shortcode .= '<div class="product-description">'.get_the_content().'<div class="clearfix"></div><span class="desc-arrow"></span></div>';
